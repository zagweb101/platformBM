...
export default function AdminCourseBuilderClient({
  ...
  const [activeLessonSectionId, setActiveLessonSectionId] = useState<
    string | null
  >(null);
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-grow">
          ...
          <p className="text-sm text-text-secondary">
            قم بتعديل بيانات الدورة وبناء المنهج التعليمي (أقسام ودروس).
          </p>
        </div>
        <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-brand-indigo/10 text-brand-indigo text-xs font-bold">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="card-brand bg-card p-6 space-y-4">
            <h3 className="text-base font-bold text-text-primary border-b border-subtle pb-2">
              تفاصيل الدورة
            </h3>

            <form
              onSubmit={handleUpdateCourseDetails}
              className="space-y-4"
            >
              <div>
          ...
          <div className="space-y-4">
            ...
            {course.sections.length === 0 ? (
              ...
              course.sections.map((section) => (
                <div
                  key={section.id}
                  className="card-brand bg-card p-6 space-y-4"
                >
                  {/* Section Header */}
                  <div className="flex items-center justify-between border-b border-subtle/50 pb-3">
                    {editingSectionId === section.id ? (
                      <div className="flex items-center gap-2 flex-grow max-w-md">
                        ...
                        <button
                          onClick={() =>
                            handleUpdateSectionTitle(section.id)
                          }
                  ...
                  <div className="space-y-2.5">
                    {section.lessons.map((lesson) => (
                      <div
                        key={lesson.id}
                        className="p-3.5 rounded-xl border border-subtle bg-secondary/30 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-brand-violet/20 transition-all"
                      >
                    ...
                    {activeLessonSectionId === section.id ? (
                      ...
                      <button
                        onClick={() =>
                          setActiveLessonSectionId(section.id)
                        }
